...
- **Role-Based Access**:
  ...
  - `POST`, `PUT`, `DELETE /api/v1/admin/**`: Require `ROLE_ADMIN` (or `ADMIN` authority based on project mapping).
  - `POST`, `PUT`, `DELETE /api/v1/organizer/**`: Require `ROLE_ORGANIZER` (or `ORGANIZER` authority).
...
- **Package**: `com.example.swp.features.<feature_name>` (e.g., `com.example.swp.features.hackathon_event`).
...
### 2. DTOs & Mappers
- **Package DTOs**: `com.example.swp.features.<feature_name>.dto.request` and `com.example.swp.features.<feature_name>.dto.response`.
- **Naming convention**: `<Entity>RequestDto` and `<Entity>ResponseDto` (or custom request/response objects like `Create<Entity>Request`).
- **Mapper**: MapStruct interface `<Entity>Mapper` with `componentModel = "spring"`.
- *Note*: Ensure the MapStruct dependency is added to `pom.xml` if active. If not using MapStruct, write manual mapper methods in the service implementation using builder patterns.

### 3. Service & Controller
- **Repository**: `<Entity>Repository` extending `JpaRepository<Entity, Long>`.
- **Service**: `<Entity>Service` interface and `<Entity>ServiceImpl` implementation.
- **Controller**: `<Entity>Controller` mapped at `/api/v1/<entities>`. Use `@PreAuthorize` for specific authority/role access controls.
